...
import type { DashboardOverview, TimelineItem, RankingItem } from '@/types/dashboard';
...
function randFloat(a: number, b: number, d = 1) {
  const p = Math.pow(10, d);
  return Math.round((Math.random() * (b - a) + a) * p) / p;
}

// Internal mutable state copied from dashboardMock
const state: DashboardOverview = JSON.parse(JSON.stringify(dashboardMock));
let tick = 0;
...
function updateSummary() {
  ...
  if (healthKpi) {
    ...
    // occasionally dip
    if (Math.random() < 0.05) next = clamp(next - randFloat(0.5, 3, 1), 92, 99.9);
...
function updateTrend() {
  ...
  const chart = state.charts.trend as any;
  const xAxis = chart.xAxis = chart.xAxis || {};
  const series = (chart.series && (chart.series as any)[0]) as any;
  const now = new Date();
  const label = now.getHours().toString().padStart(2, '0') + ':' + now.getMinutes().toString().padStart(2, '0');

  const visits = (series.data as number[]).slice();
  ...
  const labels = (xAxis.data as string[] | undefined) ?? [];
  labels.push(label);
...
function updateRanking() {
  ...
  if (tick % randInt(4, 5) !== 0) return;
  state.ranking.forEach((r: RankingItem) => {
...
function updateStructure() {
  ...
  if (tick % randInt(5, 8) !== 0) return;
  const series = ((state.charts.structure as any).series && (state.charts.structure as any).series[0]) as any;
  const data = ((series && series.data) || []).map((d: any) => Number(d.value));
  const deltas = data.map(() => randInt(-3, 3));
    let next = data.map((v: number, i: number) => clamp(v + (deltas[i] ?? 0), 1, 80));
    const sum = next.reduce((s: number, x: number) => s + x, 0);
    next = next.map((v: number) => Math.round((v / Math.max(1, sum)) * 100));
    let fix = 100 - next.reduce((s: number, x: number) => s + x, 0);
    next[0] = (next[0] ?? 0) + fix;
  series.data = series.data.map((d: any, i: number) => ({ ...d, value: next[i] }));
}

function updateRadar() {
  // low frequency: every ~15 ticks (~30s)
  if (tick % 15 !== 0) return;
  const series = ((state.charts.healthRadar as any).series && (state.charts.healthRadar as any).series[0]) as any;
...
export function nextDashboardFrame(): DashboardOverview {
  tick++;
  updateSummary();
  updateTrend();
  updateTimeline();
...
export function getCurrentFrame(): DashboardOverview {
  return JSON.parse(JSON.stringify(state));
}
